...
/**
 * @name	/nieuws.tpl
 * @version	1.0 2013-10-14 13:43:02
 *
 * @usage:	<?php $this->includeTpl('/nieuws.tpl'); ?>
 */
?>
...
<?php $this->includeTpl('includes/header.tpl'); ?>
					...
					<div id="content" class="row" data-equalizer>
						...
						<div id='column-right' class="columns large-17 medium-24" data-equalizer-watch>
							...
							<h1>Osteopathisch nieuws</h1>							
							

							<?php if(isset($this->Nieuws) && $this->Nieuws): ?> 
								<?php foreach($this->Nieuws as $bericht):?>
							
								<h4><?=$this->format($bericht['datum'], 'date', '%d %B %Y')?></h4>
								<?php if($bericht['body_teaser']): ?>
									<h3 class='nomargin'><a href='<?=APPLICATION_URL_SHORT?>/nieuws/<?=$bericht['short_url']?>'><?=$bericht['titel']?></a></h3>
								<?php else: ?>
								...
								<?php if($bericht['body_teaser']): ?>							
									<p><?=$bericht['body_teaser']?><br /><a href='<?=APPLICATION_URL_SHORT?>/nieuws/<?=$bericht['short_url']?>'>Lees meer</a></p>
								<?php else: ?>
									<?=$bericht['body']?>
								<?php endif;?>
								<?php endforeach; ?>
