...
function JTMLTemplate( template ){
	...
	var divtpl = $("div.template."+template).html();
	var scripttpl = $("script.template."+template).html();
	var tplsrc = '';
	...
	this.template = tplsrc.replace(/&amp;/gi, '&');
	
	
	//this.tplname = template;
	//if(this.tplname == 'basket-infotip'){
//	}
	
	//var tpl = $("script.template."+template);
	//this.template = ((typeof( tpl ) === "string") ? tpl : tpl.html());
...
JTMLTemplate.prototype = {
	...
	render: function( map ){
		...
		//console.log(functionParts.join( "\n" ));
		//console.log(renderFunction( map ));
