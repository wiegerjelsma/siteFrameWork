...
class Pdf extends Library {

	private $_doc;
	private $_style;
	
	public function loadBasefile($directory, $filename){
		try {
			ini_set("memory_limit", "128M");
		
			$dir = preg_replace('/\/$/','', $directory);
			$this->_doc['source'] = Zend_Pdf::load($directory.'/'.$filename);
			$this->_doc['page'] = $this->_doc['source']->pages[0];
		
		} catch(Exception $e){
			dump('[LIBRARY PDF ERROR] :: Unable to load pdf base file | '. $e->getMessage());
			throw new Exception($e);
		}
	}
	
	public function writeElement($value, $a_Font, $a_Layout, $a_Markup){
		try {	
			$this->_style = new Zend_Pdf_Style();
		
			$this->_style->setFillColor(new Zend_Pdf_Color_HTML($a_Font['color']));
		
			// calculate the fontsize
			$font = Zend_Pdf_Font::fontWithPath($a_Font['path'].'/'.$a_Font['name']);
			$fontSize = $a_Font['size'] * 1;
			$maxWidth =  $a_Layout['maxwidth'] ? $a_Layout['maxwidth'] * 1 : false;
			if($a_Markup['wordwrap'])
				$value = $this->_wrapText($value, $maxWidth, $font, $fontSize);
			else {	
				$stringWidth = $this->_widthForStringUsingFontSize($value, $font, $fontSize);
				if($stringWidth > $maxWidth && $maxWidth)
					do {
						$stringWidth = $this->_widthForStringUsingFontSize($value, $font, $fontSize);
			...
			$this->_style->setFont($font, $fontSize);
			$this->_doc['page']->setStyle($this->_style);
				
			if($a_Markup['wordwrap']){
				$bottomOffset = $a_Layout['bottomoffset'];
				foreach($value as $val){
					$bottomOffset = $bottomOffset-$a_Markup['spaced'];
					$this->_doc['page']->drawText($val, $a_Layout['leftoffset'], $bottomOffset, 'UTF-8');			
				}
			} else {	
				$left = ($a_Markup['align'] == 'right') ? $this->_doc['page']->getWidth() - ($a_Layout['rightoffset'] + $stringWidth) : $a_Layout['leftoffset'];
 				$this->_doc['page']->drawText($value, $left, $a_Layout['bottomoffset'], 'UTF-8');
			}			
 			return true;			
			
		} catch(Exception $e){
			dump('[LIBRARY PDF ERROR] :: Unable to write element | '. $e->getMessage());
			throw new Exception($e);		
		}
	}
	...
	public function save($directory, $filename){
		try {
			$dir = preg_replace('/\/$/','', $directory);
			$this->_doc['source']->save($dir.'/'.$filename);
		} catch(Exception $e){
			dump('[LIBRARY PDF ERROR] :: Unable to save pdf file | '. $e->getMessage());
			throw new Exception($e);		
		}
	}
	...
	public function render(){
		try {
			return $this->_doc['source']->render();
		} catch(Exception $e){
			dump('[LIBRARY PDF ERROR] :: Unable to render pdf file | '. $e->getMessage());
			throw new Exception($e);		
		}	
	}
	
	
	private function _wrapText($text, $colWidth, $font, $fontSize, $encoding = 'UTF-8'){
		$strLength = $this->_widthForStringUsingFontSize($text, $font, $fontSize);
		$avg = intval($strLength / strlen($text)) + 1;	
		$numToWrap = $colWidth / $avg;		
		$text = str_replace("\n","#n#",$text);
		$text = str_replace("\r","#r#",$text);
		return explode('#r##n#', wordwrap($text, $numToWrap, '#r##n#'));
	}	
	 ...
	 * @return float
	 */
	private function _widthForStringUsingFontSize($string, $font, $fontSize, $encoding = 'UTF-8'){
    	//$docText = iconv('ISO-8859-1', 'ASCII//TRANSLIT', $docText);
		
		$drawingString = iconv($encoding, 'UTF-16BE', $string);
    	$characters = array();
